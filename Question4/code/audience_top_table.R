audience_top_table <- function(netflix, Latex = TRUE){
    ...
    top_directors <- netflix %>%
        ...
        group_by(directors) %>%
        summarise(avg_score = median(tmdb_score)) %>%
        arrange(desc(avg_score)) %>%
        rename("IMDB Audience Score"= "avg_score") %>%
        slice_head(n=20)
    ...
    top_actors <- netflix %>%
        mutate(year = make_date(release_year, 1, 1)) %>%
        select(actors,type, tmdb_score, year) %>%
