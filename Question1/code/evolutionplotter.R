evolutionplotter <- function(covid){
    ...
    evolution <- covid %>%
        ...
        summarise(avg_cases = mean(total_cases)/100000 ) %>%
        ungroup()
    ...
    evolutionplot <- evolution %>%
        ...
        geom_text(data = evolution %>% filter(date == max(date) %m-% months(2)), aes(label = continent, color = continent, x = date, y = avg_cases), vjust = 0.5, nudge_y = 0.4, size = 3) +
        theme_bw() +
        labs(title = "The evolution of Total Cases of COVID-19", subtitle = "by Continent", x = "Date", y = "Total cases (in Millions)", caption = "Data source: Our World in Data") +
